...
const { APP_PORT, DATABASE_HOST, DATABASE_PORT, DATABASE_NAME, WEATHER_API_KEY, WEATHER_API_URI} = process.env;
...
async function fetchWeather(cityName)
{
  ...
  try
  {
    const resp = await fetch( `${WEATHER_API_URI}?q=${cityName}&appid=${WEATHER_API_KEY}`)
...
app.get("/", async (req, res) => {
  ...
  const {main:{temp}, name, weather} = await fetchedWeather("Managua")
  //then store the temperature in the database and return that info
  last = {value: temp, city: name, weather: weather[0], weather_description: weather[0].description}
...
app.listen(APP_PORT, () => console.log(`Server is listening at port: ${APP_PORT}`))
